...
export default function FreeCourse({ answer }) {
        ...
        <section className="text-gray-600 body-font mt-32 " >
          <div className="container px-5  mx-auto ">
            <div className="">
              ...
              {/* <div className="h-1 w-20 bg-indigo-500 rounded mx-24"></div> */}
            </div>
            <div className=''>
             ...
             {searchkey ? (<h1 className=' text-center text-3xl'>Not Found</h1>):
              ...
              {
                answer.filter((obj) => obj.title.toLowerCase().includes(searchkey)).map((item) => {
                  return (
                    <div className="xl:w-1/4 md:w-1/2 p-10 py-1 md:p-4 mb-10  " key={item._id} >
                      <Link href={`/answers/${item._id}`}> <div className="bg-gray-200 p-5 rounded-2xl  hover:shadow-2xl hover:shadow-[#6366F1] hover:ring-offset-1 hover:ring-1  " style={{ backgroundColor: mode === 'dark' ? 'rgb(31 41 55)' : '', color: mode === 'dark' ? 'white' : '' }} >
                        ...
                        <h2 style={{ color: mode === 'dark' ? 'white' : '' }} className="text-lg text-gray-900 font-medium title-font mb-4">{item.title.substr(0, 26)}</h2>
                        <p className="leading-relaxed text-base text-justify" >{item.description.replace(/<[^>]+>/g, '').substr(0, 60)}</p>
...
export async function getServerSideProps(context) {
  ...
  return {
    props: { answer }, // will be passed to the page component as props
  }
}